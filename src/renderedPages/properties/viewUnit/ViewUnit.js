...
const ViewUnit = () => {
  ...
  const [updateUnit, { isLoading: updateunitLoading }] =
    useUpdateUnitMutation();
  ...
  useEffect(() => {
    if (unit) {
      setEditableUnit({
        ...
        annual_fee: unit?.annual_fee,
        date: unit?.next_payment_date,
      });
    }
  }, [unitLoading]);
  ...
  const editUnit = (e) => {
    ...
    if (
      editableunit?.name &&
      editableunit?.tenant_id &&
      editableunit?.annual_fee
    ) {
  ...
  const doDeleteUnit = (e) => {
    e.preventDefault();
    window.confirm("Are you sure you want to delete this unit?") &&
      deleteUnit(unitId);
    navigate("/properties/viewProperty?id=" + unit?.property_id);
  };

  return unitLoading ? (
    <h1 className="title">Loading Unit Please wait....</h1>
  ) : updateunitLoading ? (
    <h1 className="title">Updating Unit Please wait....</h1>
  ) : (
    <div className="unit">
        ...
        <div className="content">
          <div className="input-content">
            <div className="input small">
              ...
              <input
                ...
                onChange={(e) =>
                  setEditableUnit({ ...editableunit, name: e.target.value })
                }
              />
            </div>
            <div className="input small">
            ...
            <div className="input small">
              ...
              <input
                ...
                onChange={(e) =>
                  setEditableUnit({ ...editableunit, date: e.target.value })
                }
              />
            </div>
            <div className="input small">
              <button className={"submit"} onClick={editUnit}>
                Update
              </button>
            </div>
          </div>
          ...
          <div className="card-container">
            <div
              ...
              key={unit?.id}
              onClick={() =>
                navigate("/tenants/viewTenant?id=" + unit?.tenant_id)
              }
            >
