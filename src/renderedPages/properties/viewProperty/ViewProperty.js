...
import { useGetSinglePropertyQuery, useUpdatePropertyMutation, useCreateUnitMutation, useDeletePropertyMutation } from "../propertyApiSlice";
...
const ViewProperty = () => {
  ...
  const [createUnit, { isLoading: createUnitLoading }] = useCreateUnitMutation();
  const [deleteProperty, { isLoading: deletePropertyLoading }] = useDeletePropertyMutation();
  ...
  const doDeleteProperty = (e) => {
    e.preventDefault();
    window.confirm("Are you sure you want to delete this property?") && deleteProperty(propertyId);
    navigate('/properties');
  };
  ...
  return (
    ...
    <div className='property'>
      <div className='top-header'>
        <span></span>
        <button style={{backgroundColor:'red'}} className={'submit'} onClick={doDeleteProperty}>Delete Property</button>
      </div>
        ...
        <div className='content'>
          ...
          <div className='input small'>
            ...
            <input 
                ...
                onChange={(e) => setUnit({...unit, annual_fee: e.target.value})}
                />
          </div>
          <div className='input small'>
            <label>Tenant</label>
            <select value={unit.tenant_id} onChange={(e) => setUnit({...unit, tenant_id: e.target.value})}>
          ...
          <div className='input small'>
          <button className={'submit'} onClick={addUnit}>Add</button>
          </div>
          </div>
          <div className="title">
            <h4>Units</h4>
