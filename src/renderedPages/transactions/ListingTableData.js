...
const ListingTableRow = (props) => {
  ...
  const termFound = (transaction) =>{
    ...
    if (transaction?.tenant?.name?.toLowerCase().includes(props.searchTerm.toLowerCase())) {
      return true;
    }
    if (transaction?.unit?.name?.toLowerCase().includes(props.searchTerm.toLowerCase())) {
      return true;
    }
    if (transaction?.type?.toLowerCase().includes(props.searchTerm.toLowerCase())) {
      return true;
    }
    if (showDate(transaction?.created_at).toLowerCase().includes(props.searchTerm.toLowerCase())) {
      return true;
    }
    return false;
  }
    ...
    <tbody className="tbody">
      {transactions?.map((transaction, index) => (
        ...
        <tr key={transaction.id} className="tr--container">
          ...
          <td data-label="Amount" className="table--amount ">{transaction.amount}</td>
